...
export async function computeCompetitorDna(
  ...
  const { data: cits } = await supabase
    .from("citations")
    .select("run_id, cited_url, cited_domain, is_own_domain, is_trusted_source")
    .in("run_id", runIds);
  ...
  // Content velocity — distinct cited URLs across the recent run window.
  // citations carry no timestamp, so recency comes from the run window this
  // query already loads (latest runs, ordered by run_at); scaled so ~20
  // distinct pages ≈ 100.
  if (domainLower) {
