...
export async function executeStep(
  ...
  try {
    switch (step.kind) {
      case "discover": {
        // prompts stores the text in `text`; alias to `question` so the
        // step output keeps its historical shape.
        const { data, error } = await sb
          .from("prompts")
          .select("id, question:text, language")
          .eq("brand_id", brandId)
      ...
      case "analyze": {
        // visibility_runs has no brand_id/created_at — it links to a brand
        // via prompt_id and timestamps with run_at. Resolve the brand's
        // prompt ids first, then filter runs by them.
        const { data: promptRows } = await sb
          .from("prompts")
          .select("id")
          .eq("brand_id", brandId);
        const promptIds = (promptRows ?? []).map((p) => (p as { id: string }).id);
        if (promptIds.length === 0) {
          result = { stepId: step.id, state: "executing", output: { runs: [] } };
          break;
        }
        const { data, error } = await sb
