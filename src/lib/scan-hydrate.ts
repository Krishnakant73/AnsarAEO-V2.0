...
export async function hydrateScanIntoBrand(params: {
  ...
  // 1. Brand.
  //
  // `slug` is NOT NULL + unique-per-org (brands_org_slug_idx). Generate a
  // unique slug from the brand name — same contract the onboarding path
  // uses (src/app/api/onboarding/route.ts). Without this the insert 500s at
  // signup scan hydration.
  ...
  const slug = uniqueSlug(
    brandName,
    (existingBrands ?? []).map((b) => b.slug as string).filter(Boolean),
  );
