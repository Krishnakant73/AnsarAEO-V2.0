...
const brandWorkspace = defineWorkspace<Brand>({
  ...
  timeline: (b) => ({
    async entries() {
      ...
      const [runsRes, contentRes, tasksRes] = await Promise.all([
        supabase
          .from("visibility_runs")
          .select("id, run_at, brand_mentioned, prompt_id, engines(name)")
          .in(
            "prompt_id",
            (
        ...
        supabase
          ...
          .limit(40),
        // tasks link to a brand via their mission (no tasks.brand_id column).
        supabase
          .from("tasks")
          .select("id, created_at, type, title, status")
          .in(
            "mission_id",
            (
              await supabase.from("missions").select("id").eq("brand_id", b.id).limit(200)
      ...
      const rows: Row[] = [];
      for (const r of (runsRes.data as unknown as { id: string; run_at: string; engines: { name: string } | null; brand_mentioned: boolean | null }[]) ?? []) {
        rows.push({
          id: `run-${r.id}`,
          at: r.run_at,
          kind: "scan",
          message: `${r.engines?.name ?? "Engine"} scan · ${r.brand_mentioned ? "mentioned" : "not mentioned"}`,
          href: `/dashboard/b/${b.slug}/history`,
        });
      }
      ...
      for (const t of (tasksRes.data as { id: string; created_at: string; type: string; title: string; status: string }[]) ?? []) {
        rows.push({
          id: `task-${t.id}`,
          at: t.created_at,
          kind: t.type ?? "task",
    ...
    async entries() {
      ...
      const [firings, runs] = await Promise.all([
        supabase
          ...
          .select("id, fired_at, metric, metric_value, threshold")
          .eq("brand_id", b.id)
          .order("fired_at", { ascending: false })
          .limit(6),
        supabase
          .from("visibility_runs")
          .select("id, run_at, brand_mentioned, engines(name)")
          .in(
            "prompt_id",
            (
      ...
      for (const f of (firings.data as { id: string; fired_at: string; metric: string }[]) ?? []) {
        rows.push({
          id: `alert-${f.id}`,
          at: f.fired_at,
          kind: "alert",
      ...
      for (const r of (runs.data as unknown as { id: string; run_at: string; engines: { name: string } | null; brand_mentioned: boolean | null }[]) ?? []) {
        rows.push({
          id: `run-${r.id}`,
          at: r.run_at,
          kind: "scan",
          message: `${r.engines?.name ?? "Engine"} · ${r.brand_mentioned ? "mentioned" : "not mentioned"}`,
        });
      }
      rows.sort((a, b) => (a.at < b.at ? 1 : a.at > b.at ? -1 : 0));
  ...
  related: (b) => ({
    async nodes() {
      ...
      for (const c of (camps.data as { id: string; name: string }[]) ?? []) {
        nodes.push({ kind: "campaign", id: c.id, label: c.name, relation: "active_campaigns" });
      }
      return nodes;
    },
  }),
